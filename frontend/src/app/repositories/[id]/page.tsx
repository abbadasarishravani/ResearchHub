...
import { FileText, ArrowLeft, FolderOpen, MessageSquare, Edit, Trash2, ExternalLink, Calendar, Code, Users } from 'lucide-react';
...
export default function RepositoryDetailPage() {
  ...
  const fetchRepository = async () => {
    try {
      const data = await api.repositories.getOne(params.id as string);
      setRepository(data);
    } catch (error) {
  ...
  const handleDelete = async () => {
    ...
    try {
      await api.repositories.delete(params.id as string);
      router.push('/papers');
    } catch (error) {
            ...
            <CardContent className="p-6">
              ...
              {repository.codeReviews?.length === 0 ? (
                ...
                <div className="space-y-3">
                  {repository.codeReviews.map((review: any, index: number) => (
                    <motion.div
                      key={review.id}
                      initial={{ opacity: 0, x: -20 }}
                      animate={{ opacity: 1, x: 0 }}
                      transition={{ delay: index * 0.1 }}
                      className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-900/50 rounded-xl hover:bg-gray-100 dark:hover:bg-gray-900 transition-colors cursor-pointer"
                    >
