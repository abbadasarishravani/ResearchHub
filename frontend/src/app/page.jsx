'use client';
// Trigger build for Vercel Root Directory fix
...
import { Sparkles, CheckCircle2, Brain, TrendingUp, MessageSquare, Star, ChevronRight, Zap, Shield, Users, FileText, Plus, ArrowRight, BarChart3 } from 'lucide-react';
    ...
    return (<div className="min-h-screen bg-background flex flex-col">
      ...
      <main className="flex-grow">
        ...
        <section className="relative pt-32 pb-24 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto overflow-hidden">
          ...
          <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="text-center relative z-10">
            ...
            {!isAuthenticated && (<div className="flex flex-col sm:flex-row gap-6 justify-center items-center">
        ...
        {isAuthenticated && user && (<section className="py-20 bg-accent/[0.03] border-y border-border">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-12 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                <div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                { label: 'Published', val: stats.published, color: 'text-green-500' },
            ].map((s, i) => (<div key={i} className="bg-background rounded-2xl p-6 border border-border shadow-sm text-center">
                    ...
                    {papers.length === 0 ? (<div className="text-center py-12 bg-background/40 rounded-2xl border-2 border-dashed border-border">
                        ...
                        {papers.slice(0, 3).map((paper) => (<Link key={paper.id} href={`/papers/${paper.id}`}>
        ...
        ].map((f, i) => (<motion.div key={i} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: i * 0.1 }} viewport={{ once: true }} className="group p-8 rounded-3xl bg-card border border-border hover:border-accent transition-all duration-300 shadow-sm hover:shadow-xl hover:-translate-y-2">
        ...
        <section id="how-it-works" className="py-32 px-4 sm:px-6 lg:px-8 bg-secondary/30 border-y border-border scroll-mt-20">
          <div className="max-w-7xl mx-auto">
            ...
            { n: '4', t: 'Publish', d: 'Finalize your research and publish accepted work to the hub.' },
        ].map((s, i) => (<div key={i} className="text-center relative">
                  ...
                  {i < 3 && (<div className="hidden lg:block absolute top-10 left-[60%] w-[80%] h-0.5 bg-accent/20 -z-0"/>)}
                </div>))}
            </div>
          </div>
        </section>
        ...
        ].map((t, i) => (<motion.div key={i} initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} transition={{ duration: 0.5, delay: i * 0.1 }} viewport={{ once: true }} className="bg-card p-10 rounded-[2.5rem] border border-border shadow-sm hover:shadow-xl transition-all duration-300">
        ...
        {/* 7. CTA Section */}
        {!isAuthenticated && (<section className="py-32 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
            <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} viewport={{ once: true }} className="relative text-center bg-gradient-to-br from-accent to-blue-600 rounded-[3rem] p-16 sm:p-24 border border-white/10 shadow-2xl overflow-hidden">
