...
import { type PaperStatus, type PaperCategory } from '@/types';
...
export default function PapersPage() {
  ...
  const [selectedCategory, setSelectedCategory] = useState<PaperCategory | 'all'>('all');
  const [selectedStatus, setSelectedStatus] = useState<PaperStatus | 'all'>('all');
  ...
  const categories: PaperCategory[] = ['ai', 'ml', 'nlp', 'cv', 'data-science', 'other'];
  const statuses: PaperStatus[] = ['draft', 'submitted', 'under-review', 'accepted', 'rejected'];
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
            ...
            <div className="flex gap-2 justify-end">
              <button
                ...
                className={`p-2 rounded-md ${viewMode === 'grid' ? 'bg-accent text-white' : 'bg-secondary'}`}
              >
                <Grid className="w-4 h-4" />
              </button>
              <button
                onClick={() => setViewMode('list')}
                className={`p-2 rounded-md ${viewMode === 'list' ? 'bg-accent text-white' : 'bg-secondary'}`}
              >
        ...
        {sorted.length === 0 ? (
          <motion.div
            ...
            className="flex flex-col items-center justify-center py-16 px-4 text-center bg-card rounded-2xl border border-dashed border-border shadow-sm"
          >
