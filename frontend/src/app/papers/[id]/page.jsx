...
import { ArrowLeft, Edit2, Trash2, Download, Eye } from 'lucide-react';
...
export default function PaperDetailPage({ params }) {
    ...
    return (<div className="min-h-screen bg-background">
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* Back Button */}
        <motion.button initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} onClick={() => router.back()} className="flex items-center gap-2 text-muted-foreground hover:text-foreground transition-colors mb-6">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
          ...
          <div className="flex flex-wrap gap-2 mb-4">
            ...
            {paper.tags.map((tag) => (<Badge key={tag} variant="outline">
                {tag}
              </Badge>))}
          </div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="mb-8">
          <Card>
            ...
            <CardContent>
              {paperReviews.length === 0 ? (<p className="text-muted-foreground">No reviews yet</p>) : (<div className="space-y-6">
                  {paperReviews.map((review) => (<div key={review.id} className="border-b border-border pb-6 last:border-0">
                      <div className="flex items-start justify-between mb-3">
                        ...
                        <div className="flex gap-1">
                          {[...Array(5)].map((_, i) => (<span key={i} className={`text-lg ${i < review.rating ? 'text-yellow-400' : 'text-muted-foreground'}`}>
                              ★
                            </span>))}
                        </div>
                      </div>
